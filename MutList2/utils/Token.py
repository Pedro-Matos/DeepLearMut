...
class Token:
    ...
    _prefixPattern = re.compile(
        "(" + _locantRe + "(," + _locantRe + ")*)" + "[" + StringTools.hyphens + "](\\S*)" + "$")  ###
    ...
    def splitAt(self, splitOffset0, splitOffset1=None):
        ...
        if splitOffset1 == None:
            ...
            tokens.append(
                Token(self.value[internalOffset0:internalOffset1], splitOffset0, splitOffset1, self.tokeniser))
    ...
    def _splitInternal(self, clm=False):
        ...
        if firstchar in "([{" and (
                StringTools.isBracketed(self.value) or StringTools.isLackingCloseBracket(self.value)):
        ...
        if ":" in middleValue:
            ...
            if StringTools.bracketsAreBalanced(self.value) and StringTools.bracketsAreBalanced(
                    self.value[self.value.find(":") + 1:]):
                return self.splitAt(self.start + self.value.find(":"), self.start + self.value.find(":") + 1)
        if "+" in middleValue:
            ...
            if index > 0 and index < len(self.value) - 1:
                if lastchar == "-":
                    pass
                elif StringTools.bracketsAreBalanced(self.value) and StringTools.bracketsAreBalanced(
                        self.value[index + 1:]):
                    return self.splitAt(self.start + index, self.start + index + 1)
                else:
                    return self.splitAt(self.start + index, self.start + index + 1);
        if StringTools.midElipsis in middleValue:
            return self.splitAt(self.start + self.value.find(StringTools.midElipsis),
                                self.start + self.value.find(StringTools.midElipsis) + 1)
        if clm and "=" in middleValue:
